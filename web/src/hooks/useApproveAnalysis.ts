...
import { approveAnalysis } from '@/services/analysis'

export function useApproveAnalysis(analysisId: string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: () => approveAnalysis(analysisId),
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      if (errorMessage.includes('409') || errorMessage.toLowerCase().includes('already')) {
        ...
        toast.error(errorMessage || 'Failed to approve analysis. Please try again.')
      }
    },
  })
}
